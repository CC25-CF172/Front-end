...
export default function StuntWatchLogin() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        // Simpan token dan nama user ke localStorage/sessionStorage
        localStorage.setItem("token", data.data.token);
        localStorage.setItem("fullname", data.data.fullname); // Pastikan backend mengirim nama lengkap
        setTimeout(() => navigate("/"), 1000); // Redirect ke home
      } else {
  ...
  const handleGoogleLogin = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/auth/callback/google`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: 'include',
        body: JSON.stringify({
          provider: "google",
          user: {
            email, // gunakan email yang diinput user
          },
        }),
      });
      const data = await response.json();
        ...
        <div className="mt-8 space-y-6">
          ...
          <div>
            <button
              ...
              className="w-full inline-flex justify-center py-3 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 transition duration-150 ease-in-out"
            >
